...
namespace ViFactory.Api.Extentions
{
    public static class DiDbContext
    {
        ...
        {
            services.AddDbContext<SamplePDbContext>(x =>
            {
                x.UseNpgsql(configuration.GetConnectionString("PostreSql"), config =>
                {
                    config.MigrationsAssembly("SamplePDbContext.Dal");
                });
            });
            ...
            }).AddDefaultTokenProviders().AddEntityFrameworkStores<SamplePDbContext>();
