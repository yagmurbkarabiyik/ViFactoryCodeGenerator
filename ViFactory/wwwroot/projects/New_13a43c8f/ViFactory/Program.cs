...
class Program
{
    ...
    static void GenerateDal(string csprojFilePath, string rootFolderPath, IEnumerable<Type> entities, string currentProjectName)
	{
		...
		var customConstructorText = currentContextText.Substring(currentContextText.IndexOf("#region CustomConstructor"), currentContextText.LastIndexOf("#endregion CustomConstructor") + 1);

        var customOnModelCreatingText = currentContextText.Substring(currentContextText.IndexOf("#region CustomOnModelCreating"), currentContextText.LastIndexOf("#endregion CustomOnModelCreating") + 1);

        var customText = currentContextText.Substring(currentContextText.IndexOf("#region CustomOnModelCreating"), currentContextText.LastIndexOf("#endregion CustomOnModelCreating") + 1);
	...
	static void GenerateApiController(string csprojFilePath, string rootFolderPath, IEnumerable<Type> entities, string currentProjectName)
	{
        ...
        foreach (var entity in entities)
        {
            string controllerClass = Path.Combine(apiControllerFolderPath, $"{entity.Name}Controller.cs");
            ...
            if (entity.Name != "BaseEntity")
            {
                EnsureFileExists(controllerClass, File.ReadAllText("./Templates/ApiController.txt")
                    ...
                    .Replace("[EntityName]", $"{char.ToLower(entity.Name[0])}{entity.Name.Substring(1)}")
);
            }
        }
    }
