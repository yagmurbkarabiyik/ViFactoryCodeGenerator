...
using SampleProject.Bll.Services.Abstract;
namespace SampleProject.Apps.App.Api.Middlewares
{
    public class ApiContextMiddleware
    {
        ...
        //**
        public async Task Invoke(HttpContext context, ILanguageService languageService, ApiContext apiContext)
        {
            ...
            var language = context.Request.Headers["X-Client-Language"];
            if (!string.IsNullOrEmpty(language) && language.Count > 0)
                apiContext.Language = language.FirstOrDefault() ?? await languageService.GetDefaultCode();
            else
                apiContext.Language = await languageService.GetDefaultCode();
