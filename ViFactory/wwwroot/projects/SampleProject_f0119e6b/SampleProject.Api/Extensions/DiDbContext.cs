...
namespace ViFactory.Api.Extentions
{
    public static class DiDbContext
    {
        ...
        {
            services.AddDbContext<SampleProjectDbContext>(x =>
            {
                x.UseNpgsql(configuration.GetConnectionString("PostreSql"), config =>
                {
                    config.MigrationsAssembly("SampleProjectDbContext.Dal");
                });
            });
            ...
            }).AddDefaultTokenProviders().AddEntityFrameworkStores<SampleProjectDbContext>();
