...
using Deneme.Core.Models.TokenModels;
using Deneme.Domain.Dtos.AppUserDtos;
using Deneme.Domain.Entities.IdentityModels;

namespace Deneme.Bll.Services
{
    /// <summary>
    ...
    public partial class AppUserService
    {
        ...
        public AppUserService(UserManager<AppUser> userManager, SignInManager<AppUser> signInManager, IAppUserRepository appUserRepository, IUnitOfWork unitOfWork, IUploadLocalService uploadLocalService, ApiContext apiContext, IEmailService emailService, ISmsNetGsmService smsNetGsmService, IDataProtectionProvider dataProtectorProvider, IHttpContextAccessor httpContextAccessor, ITokenService tokenService, IOptions<JwtSettings> jwtSettings)
        {
        ...
        public async Task<ResponseCommon> SendEmailPasswordResetCode(AppUserSendEmailPasswordResetCodeRequestDto dto)
        {
            ...
            var result = await _emailService.SendAsync(new EmailSendData
            {
            ...
            {
                await _smsNetGsmService.SendAsync(new SmsNetGsmSendData
                {
                    GsmNo = appUser.PhoneNumber,
                    Message = code,
                });
            }
