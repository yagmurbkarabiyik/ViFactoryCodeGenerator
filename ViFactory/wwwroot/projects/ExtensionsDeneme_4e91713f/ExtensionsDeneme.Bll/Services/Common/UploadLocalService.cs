...
using System.Net;
using ExtensionsDeneme.Bll.Models;
using ExtensionsDeneme.Core.Core.Services;

namespace ExtensionsDeneme.Bll.Services.Common
{
     public class UploadLocalService : IUploadLocalService
     {
        ...
        public async Task<List<string>> MultiUpload(List<IFormFile> files, string folderPath, int? width = null, int? height = null, int? quality = 100, bool isPrivate = false)
        {
            var result = new List<string>();

            foreach (var image in files.Where(x => x is not null))
            {
                var imageUrl = await Upload(image, folderPath, width, height, quality, isPrivate);
        ...
        public Task<string> Upload(IFormFile file, string folderPath, int? width = null, int? height = null, int? quality = 100, bool isPrivate = false)
        {
            ...
            return Upload(ConvertIFormFileToBase64(file), folderPath, width, height, quality, isPrivate);
        }

        public async Task<string> Upload(string base64, string folderPath, int? width = null, int? height = null, int? quality = 100, bool isPrivate = false)
        {
