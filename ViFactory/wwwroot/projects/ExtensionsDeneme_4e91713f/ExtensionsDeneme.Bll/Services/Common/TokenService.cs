...
using ExtensionsDeneme.Core.Services.TokenService;
...
namespace ExtensionsDeneme.Bll.Services.Common
{
    public class TokenService  : ITokenService
    {
        ...
        public GetClientTokenResult GetClientToken(GetClientTokenRequest request)
        {
            var jwtSecurityToken = GetJwtSecurityClientToken(request);

            return new GetClientTokenResult
            {
                AccessToken = GetAccessToken(jwtSecurityToken),
                AccessTokenUnixExpire = new DateTimeOffset(jwtSecurityToken.ValidTo, TimeSpan.Zero).ToUnixTimeSeconds(),
                AccessTokenExpire = jwtSecurityToken.ValidTo,
        ...
        private JwtSecurityToken GetJwtSecurityClientToken(GetClientTokenRequest request)
        {
            var accessTokenExpire = DateTimeOffset.Now.AddMinutes(request.AccessTokenExpirationMinutes);
            var now = DateTimeOffset.Now;
