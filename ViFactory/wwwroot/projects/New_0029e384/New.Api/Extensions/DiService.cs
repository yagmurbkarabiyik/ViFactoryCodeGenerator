...
using New.Bll.Services.Common;
using New.Core.Services;
using New.Core.Models;
using New.Core.Services.TokenService;
using New.Dal.Data.Common;
...
namespace ViFactory.Api.Extentions
{
    public static class DiServices
    {
        public static void AddCustomDiServices(this IServiceCollection services, IWebHostEnvironment env, IConfiguration config)
        {
            ...
            var bllServices = Assembly.Load("New.Bll")
                .GetTypes()
                .Where(x => !string.IsNullOrEmpty(x.FullName) && x.FullName.StartsWith("New.Bll.Services.Abstract."))
                .ToList();
            ...
            var dalrepositories = Assembly.Load("New.Dal")
                .GetTypes()
                .Where(x => !string.IsNullOrEmpty(x.FullName) && x.FullName.StartsWith("New.Dal.Data.IDalRepos."))
                .ToList();
